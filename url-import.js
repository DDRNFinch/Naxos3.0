(() => {
  ...
  function cleanText(text) { return String(text || '').replace(/\r/g,'').replace(/\[([^\]]+)\]\([^)]*\)/g,'$1').replace(/[*_`#>]/g,' ').replace(/\s+/g,' ').trim(); }
  function findValue(text, labels) {
    for (const label of labels) {
      const re = new RegExp(`(?:^|\\n)\\s*${label}\\s*[:\\-]?\\s*([^\\n]{1,180})`, 'im');
      const m = text.match(re); if (m) return cleanText(m[1]);
    }
    return '';
  }
  function extractKSB(text) {
    const rows=[]; const lines=String(text||'').replace(/\r/g,'').split('\n'); const seen=new Set();
    for(let i=0;i<lines.length;i++){
      const line=cleanText(lines[i]);
      const m=line.match(/\b([KSB])\s*([0-9]{1,3})\s*[:\-–—]\s*(.+)$/i); if(!m)continue;
      const ref=`${m[1].toUpperCase()}${m[2]}`; let desc=m[3].trim();
      if(desc.length<20&&lines[i+1])desc+=' '+cleanText(lines[i+1]);
      if(!seen.has(ref)){seen.add(ref);rows.push(`${ref} - ${desc}`)}
    }
    return rows;
  }
  ...
  function parseStandard(text,url){
    ...
    m.status=findValue(text,['Status']); m.route=findValue(text,['Route','Routes']); m.minimumHours=findValue(text,['Minimum hours for compliance']);
    m.maximumFunding=findValue(text,['Maximum funding']); m.larsCode=findValue(text,['LARS Code','LARS code']); m.eqaProvider=findValue(text,['EQA Provider','EQA provider']);
    m.dateUpdated=findValue(text,['Date updated']); m.approvedForDelivery=findValue(text,['Approved for delivery']); m.options=findValue(text,['Options']);
    m.regulated=findValue(text,['Regulated standard']); m.regulator=findValue(text,['Regulator body']); m.review=findValue(text,['Review']);
    m.professionalRecognition=findValue(text,['Professional recognition']); m.entryRequirements=findValue(text,['Entry requirements']); m.englishMaths=findValue(text,['English and Maths']);
  ...
  async function readStandard(){
    const url=urlInput.value.trim(); if(!/^https?:\/\//i.test(url)){setStatus('Enter a full http:// or https:// website link.','error');return;}
    ...
    try{
      ...
      const texts=[];
      for(const target of targets){const r=await fetch(`https://r.jina.ai/${target}`,{headers:{Accept:'text/plain'}});if(!r.ok)throw new Error(`Reader returned ${r.status}`);texts.push(await r.text());}
      const standard=parseStandard(texts[0],url); standard.epaPlan=extractEPA(texts.join('\n\n'));
      ...
      input.value=standard.ksbs.map(k=>k).join('\n');
